...
function ExportExcel(viewer, options) {
  Autodesk.Viewing.Extension.call(this, viewer, options);
}

ExportExcel.prototype = Object.create(Autodesk.Viewing.Extension.prototype);
ExportExcel.prototype.constructor = ExportExcel;
...
ExportExcel.prototype.load = function () {
  urn = this.options.urn.replace('urn:', '');
...
ExportExcel.prototype.onToolbarCreated = function () {
  this.viewer.removeEventListener(av.TOOLBAR_CREATED_EVENT, this.onToolbarCreatedBinded);
  this.onToolbarCreatedBinded = null;
  this.createUI();
};

ExportExcel.prototype.createUI = function () {
  var viewer = this.viewer;

  // Button 1
  var button1 = new Autodesk.Viewing.UI.Button('toolbarExportExcel');
  button1.onClick = function (e) {
    //function (access_token, expires_in) {
    ForgeXLS.downloadXLSX(urn, getToken(), null /*Optional*/);
    //});
  };
  button1.addClass('toolbarExportExcel');
  button1.setToolTip('Export to Excel (XLSX)');

  // SubToolbar
  this.subToolbar = new Autodesk.Viewing.UI.ControlGroup('myAppGroup');
...
var ForgeXLS = {
  ...
  downloadXLSX: function (urn, token, status) {
    //urn = urn.split('_')[0]; // remove version after file name
    ...
    if (fileName.indexOf('.rvt') == -1) {
      ...
      if (!confirm('Is this file a Revit file?')) {
...
Autodesk.Viewing.theExtensionManager.registerExtension('Autodesk.Sample.ExportExcel', ExportExcel);